    ...
    for i, data in enumerate(tqdm(test_dataloader)):
        ...
        if cfg.EVALUATE.INTERP != "":
            ...
            data_interp1d = torch.tensor(interp_f(
                range(0, cfg.MODEL.SLIDE_WINDOW_SIZE))).to(data_pred.device)  # (B, L, C)
        ...
        with torch.no_grad():
            predicted_pos = model(data_pred)

        # slide window to sequence
        predicted_pos = slide_window_to_sequence(predicted_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                 cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        data_pred = slide_window_to_sequence(data_pred, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                             cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        data_gt = slide_window_to_sequence(data_gt, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                           cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        if cfg.EVALUATE.INTERP != "":
            data_interp1d = slide_window_to_sequence(data_interp1d, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                     cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 2)
        data_bbox = slide_window_to_sequence(data_bbox, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                             cfg.MODEL.SLIDE_WINDOW_SIZE).type(torch.int32)
        # evaluate on jhmdb
...
def evaluate_msf_3D(model, test_dataloader, device, cfg):
    keypoint_number = eval("cfg.DATASET." + cfg.DATASET_NAME.upper() +
                           ".KEYPOINT_NUM")
    ...
    for i, data in enumerate(tqdm(test_dataloader)):
        ...
        predicted_pos = slide_window_to_sequence(predicted_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                 cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        data_pred = slide_window_to_sequence(data_pred, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                             cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        data_gt = slide_window_to_sequence(data_gt, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                           cfg.MODEL.SLIDE_WINDOW_SIZE).reshape(-1, keypoint_number, 3)
        ...
        improve_mpjpe.append(calculate_mpjpe(predicted_pos, data_gt).mean() -
                             calculate_mpjpe(data_pred[::cfg.MODEL.INTERVAL_N, ...],
                                             data_gt[::cfg.MODEL.INTERVAL_N, ...]).mean())
        improve_pampjpe.append(calculate_pampjpe(predicted_pos, data_gt).mean() -
                               calculate_pampjpe(data_pred[::cfg.MODEL.INTERVAL_N, ...],
                                                 data_gt[::cfg.MODEL.INTERVAL_N, ...]).mean())
...
def evaluate_msf_smpl(model, test_dataloader, device, cfg):
    ...
    for i, data in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            predicted_pos = model(data_pred_pose)

        predicted_pos = slide_window_to_sequence(predicted_pos, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                 cfg.MODEL.SLIDE_WINDOW_SIZE)
        data_gt_pose = slide_window_to_sequence(data_gt_pose, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                cfg.MODEL.SLIDE_WINDOW_SIZE)
        data_pred_pose = slide_window_to_sequence(data_pred_pose, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                  cfg.MODEL.SLIDE_WINDOW_SIZE)
        if cfg.EVALUATE.INTERP != "":
            data_interp1d_pose = slide_window_to_sequence(data_interp1d_pose, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                          cfg.MODEL.SLIDE_WINDOW_SIZE)

        data_pred_shape = slide_window_to_sequence(data_pred_shape, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                   cfg.MODEL.SLIDE_WINDOW_SIZE)
        if cfg.DATASET_NAME != "aist":
            data_gt_shape = slide_window_to_sequence(data_gt_shape, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                     cfg.MODEL.SLIDE_WINDOW_SIZE)
        else:
            data_gt_trans = slide_window_to_sequence(data_gt_trans, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                     cfg.MODEL.SLIDE_WINDOW_SIZE)
            data_gt_scaling = slide_window_to_sequence(data_gt_scaling, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
                                                       cfg.MODEL.SLIDE_WINDOW_SIZE)
        ...
        if cfg.EVALUATE.ROOT_RELATIVE:
            ...
            eval_smpl_result_joints = eval_smpl_result_joints - eval_smpl_result_joints[:, keypoint_root, :].mean(
                axis=1).reshape(-1, 1, 3)
            if cfg.EVALUATE.INTERP != "":
                interp_smpl_result_joints = interp_smpl_result_joints - interp_smpl_result_joints[:, keypoint_root,
                                                                        :].mean(
                    axis=1).reshape(-1, 1, 3)
        ...
        improve_mpjpe.append(calculate_mpjpe(eval_smpl_result_joints, gt_smpl_result_joints).mean() -
                             calculate_mpjpe(input_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...],
                                             gt_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...]).mean())
        improve_pampjpe.append(calculate_pampjpe(eval_smpl_result_joints, gt_smpl_result_joints).mean() -
                               calculate_pampjpe(input_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...],
                                                 gt_smpl_result_joints[::cfg.MODEL.INTERVAL_N, ...]).mean())
        improve_mpvpe.append(calculate_mpjpe(eval_smpl_result.vertices, gt_smpl_result.vertices).mean() -
                             calculate_mpjpe(input_smpl_result.vertices[::cfg.MODEL.INTERVAL_N, ...],
                                             gt_smpl_result.vertices[::cfg.MODEL.INTERVAL_N, ...]).mean())
