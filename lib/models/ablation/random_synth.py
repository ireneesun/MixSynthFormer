...
class SynthRandom(nn.Module):
    def __init__(self, in_dims, out_dims):  # L, emb
        super().__init__()

        self.attention = nn.Parameter(torch.empty(1, out_dims, out_dims), requires_grad=True)
...
class SynthMix(nn.Module):
    def __init__(self, temporal_dim, channel_dim, proj_drop):  # L, emb
        super().__init__()
        self.synth_token = SynthRandom(channel_dim, temporal_dim)
