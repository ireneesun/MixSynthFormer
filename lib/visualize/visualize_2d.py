...
def visualize_2d_jhmdb(data_imgname,
    ...
    for frame_i in trange(max(0, start_frame), min(len(data_imgname),
        ...
        img_msf = plot_kpts(img, predicted_pos[frame_i, :],
                            MSF_COLOR, JHMDB_JOINTS, JHMDB_EDGES)
        ...
        img_in = np.concatenate((img_in, np.zeros(
            (FONT_HEIGHT, imgsize_w, 3))),
                                axis=0)
        img_msf = np.concatenate(
            (img_msf, np.zeros((FONT_HEIGHT, imgsize_w, 3))), axis=0)
        ...
        img_msf = Image.fromarray(img_msf.astype(np.uint8))
