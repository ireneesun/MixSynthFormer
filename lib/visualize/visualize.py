...
class Visualize():
    ...
    def visualize_3d(self, model):
        ...
        predicted_pos = slide_window_to_sequence(predicted_pos, self.slide_window_step, self.slide_window_size).reshape(
            -1, keypoint_number, 3)
    ...
    def visualize_2d(self, model):
        ...
        data_gt = torch.tensor(data_gt).to(self.device)
        data_pred_norm = torch.tensor(data_pred.reshape(-1, 2) / data_imageshape[:2][::-1]).to(self.device).reshape_as(
            data_gt)
        data_pred = torch.tensor(data_pred).to(self.device)
