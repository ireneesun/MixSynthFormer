...
class H36MDataset(BaseDataset):
    ...
    def __init__(self, cfg, estimator='fcn', return_type='3D', phase='train'):
        ...
        # keypoints = detected_data['pose_sequence']
        # print("test npz", keypoints.shape)
        ...
        if self.return_type == '3D':
            ...
            # self.ground_truth_data_imgname = 0
            self.ground_truth_data_joints_3d = ground_truth_data["joints_3d"]
            # self.ground_truth_data_joints_3d = ground_truth_data["pose_sequence"]
            self.detected_data_imgname = detected_data["imgname"]
            # self.detected_data_imgname = 0
            self.detected_data_joints_3d = detected_data["joints_3d"]
            # self.detected_data_joints_3d = detected_data["pose_sequence"]
    ...
    def get_test_data(self, index):
        ...
        if self.slide_window_size <= ground_truth_data_len:
            start_idx = np.arange(0, ground_truth_data_len - self.slide_window_size + 1,
                                  self.evaluate_slide_window_step)
            gt_data_ = []
            pred_data_ = []
