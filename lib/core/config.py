...
############ test
BASE_DATA_DIR = '/Users/yuransun/Desktop/research/MixSynthFormer/project/data'  # data dir
...
CUSTOM_DETECTED_PATH = os.path.join(
    BASE_DATA_DIR,
    'detected_poses/custom')  # the folder for Human3.6M dataset detected poses
CUSTOM_GROUND_TRUTH_PATH = os.path.join(
    BASE_DATA_DIR, 'groundtruth_poses/custom'
)  # the folder for Human3.6M dataset groundtruth poses
...
cfg.DATASET.CUSTOM = CN()  # Human3.6M dataset config
cfg.DATASET.CUSTOM.GROUND_TRUTH_PATH = CUSTOM_GROUND_TRUTH_PATH  # Human3.6M dataset groundtruth path
cfg.DATASET.CUSTOM.DETECTED_PATH = CUSTOM_DETECTED_PATH  # Human3.6M dataset detected pose path
cfg.DATASET.CUSTOM.KEYPOINT_NUM = 17  # Human3.6M dataset keypoint number
cfg.DATASET.CUSTOM.KEYPOINT_ROOT = [0]  # Human3.6M dataset keypoint root
...
cfg.MODEL.SLIDE_WINDOW_Q = 1  # Q = frames sampled in one slide window -1
cfg.MODEL.INTERVAL_N = 10  # sampling interval N
...
# model
cfg.MODEL.BLOCK_NUM = 3  # encoder transformer block number
cfg.MODEL.EMBEDDING_DIMENSION = 512  # encoder embedding size

# training config
cfg.TRAIN = CN()
cfg.TRAIN.BATCH_SIZE = 1024  # batch size
cfg.TRAIN.WORKERS_NUM = 16  # workers number
cfg.TRAIN.EPOCH = 70  # epoch number
cfg.TRAIN.LR = 0.001  # learning rate
cfg.TRAIN.LRDECAY = 0.99  # learning rate decay rate
cfg.TRAIN.RESUME = ''  # resume training checkpoint path
...
cfg.EVALUATE.SLIDE_WINDOW_STEP_Q = 1  # slide window step
